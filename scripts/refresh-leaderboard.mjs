...
// writes the top ~22 to arena-ranking.json — which the model picker reads to
// rank the OpenRouter dropdown. Ported from recall/scripts/; the only change is
// the output path, which now targets public/ so Vite serves it as a static
// asset. Node 20+ (global fetch). Usage: node scripts/refresh-leaderboard.mjs
import fs from "node:fs";
import path from "node:path";

const ARENA_URL = "https://arena.ai/leaderboard/text";
const OR_URL = "https://openrouter.ai/api/v1/models";
const OUT = path.join(process.cwd(), "public", "arena-ranking.json");
const TOP_N = 22;
